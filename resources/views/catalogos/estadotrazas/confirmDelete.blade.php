            ...
            <div class="row">
                <div class="col">
                    <form class="bg-white shadow py-3 px-4" action="/cat_estadotrazas/{{ $et->id }}" method="POST">
                        <h3 class="text-center">Eliminar Registro: <small>{{ $et->descripcion }}</small></h3>
                        <hr>
                        {{ csrf_field() }}
                        @method('delete')
                        <br>
                        <button class="btn btn-danger btn-block" type="submit"><i class="fas fa-trash-alt"></i>&ensp;Eliminar</button>
                        <a class="class btn btn-block btn-outline-dark" href="/cat_estadotrazas"><i class="far fa-window-close"></i>&ensp;Cancelar</a>
                    </form>
                </div>
            </div>
        </div>
